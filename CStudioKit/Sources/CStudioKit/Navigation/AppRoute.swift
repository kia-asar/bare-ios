...
extension AppRoute {
    ...
    public static let scheme = "cstudio"

    /// Parse a URL into an AppRoute
    ///
    /// Supports URLs like:
    /// - `cstudio://post/{uuid}` → `.contentDetail(postId:)`
    /// - `https://cstudio.app/post/{uuid}` → `.contentDetail(postId:)` (universal links)
    ///
    ...
    public static func from(url: URL) -> AppRoute? {
        // Support both custom scheme (cstudio://) and universal links (https://cstudio.app)
        let isCustomScheme = url.scheme == scheme
        let isUniversalLink = url.scheme == "https" && url.host == "cstudio.app"
        ...
        case "post":
            // Parse post detail: cstudio://post/{uuid} or https://cstudio.app/post/{uuid}
    ...
    public func toUniversalLink() -> URL? {
        switch self {
        case .contentDetail(let postId):
            return URL(string: "https://cstudio.app/post/\(postId.uuidString)")
        }
    }
}
